...
/*
 * Introduction Paragraph: MP 11
 * In this MP, I implemented a floorplan slicing program. 
 * The slicing floorplan was represented using a tree data structure
 * Cutlines would represent how the floorplan was divided and modules representing the computer resources
 * I first implemented 3 tree functions. 
 * is_leaf_node would check if the node had any right or left nodes to establish if the current node was a leaf node
 * is_internal_node would check if the node was not a leaf node and had a child node
 * is_in_subtree would check if a certain node b was within the subtree of a node a by traversing the subtree and checking if any of the nodes matched node b
 * I then implemented 4 functions to modify the floorplans
 * rotate swaps the height and width of the specified module to rotate the module by 90 degrees
 * recut changes the cutline from horizontal to vertical or vice versa
 * swap_module would take two node pointers and swap the modules of each node
 * swap_topology would swap nodes and their subtrees. This was done by swapping pointers to the node's parents which would move the node and it's subtree
 * I then reviewed get_expression which traveresed the slicing tree in a postfix order and encoded the tree into a postfix expression
 * I had to implement postfix_traversal for this to work:
 * postfix_traversal would traverse the left and right subtrees recursively, and then process the data part of the root/current element given from get_expression
 * Next I implemented get_total_resources which recursively traversed the tree starting from a certain node. The function would use recursive calls to check left and right subtrees
 * It then found the leaf nodes and returned the total values of the leaf node modules which were the resource values
 * init_slicing_tree created a sliced tree which followed the horizontally-aligned floorplan
 * I followed the psuedocode on the wiki to slice the tree. At each recursions step, I had to generate an internal node and a right child node.
 * The internal nodes would hold cutline values and the right leaf nodes would have module pointers. 
 * When the number of modules that could be attached were zero, the recursion stopped and returned a pointer to the tree root
 *
 *
 */
...
int is_leaf_node(node_t* ptr) {
  ...
  if (!(ptr->left && ptr->right)){ //check if left node and right node is null
    return 1; //if no right or left node, current node is a leaf node
  }
  return 0;
}
...
int is_internal_node(node_t* ptr) {
  // TODO: (remember to modify the return value appropriately)
  if (!(is_leaf_node(ptr))){ //if node is not a leaf node, it is an internal node
    return 1;
  }
  return 0;
}
...
int is_in_subtree(node_t* a, node_t* b) {
  // TODO: (remember to modify the return value appropriately)
  if (a == NULL) { //base case for recursion (reached the end of the subtree)
    return 0;
  }
  if (a == b){ //if a and b are the same node, b is part of subtree
    return 1;
  }
  return (is_in_subtree(a->left, b) || is_in_subtree(a->right, b)); //use recursion: check each subtree of nodes on left and right of a
}
...
void rotate(node_t* ptr) {
  // TODO: (Swap width and height of the module)
  if (!(is_leaf_node(ptr))){ //if not a leaf node, don't rotate
    return;
  }
  int temp = ptr->module->w; //set temp to width
  ptr->module->w = ptr->module->h; //change width to height
  ptr->module->h = temp; //change height to previous width
...
void recut(node_t* ptr) {
  ...
  // TODO:
  if (ptr->cutline == V){ //check if cutline is vertical
    ptr->cutline = H; //if vertical, change to horizontal
  } else{ //if not vertical, cutline is horizontal
    ptr->cutline = V; //if horizontal, change to vertical
  }
  return;
}
...
void swap_module(node_t* a, node_t* b) {
  ...
  // TODO:
  module_t *temp = (module_t *) malloc(sizeof(module_t)); //create temp module
  temp = a->module; //set temp to module a
  a->module = b->module; //change module a to module b
  b->module = temp; //set module b to previous module a
}
...
void swap_topology(node_t* a, node_t* b) {
  ...
  // TODO:
  node_t *temp_n = (node_t *) malloc(sizeof(node_t)); //create a temporary node
  
  //first swap parent nodes
  temp_n = a->parent; //set temp to parent a
  a->parent = b->parent; //change parent a to parent b
  b->parent = temp_n; //change parent b to previous parent a

  //next swap subtrees by setting the parent's right and left nodes
  if (a->parent->left == b){ //check if node b was on parent's left or right node
    a->parent->left = a; //if on left, change that left node to the new node a
  } else{
    a->parent->right = a;//if on right, change that right node to the new node a
  }
  if (b->parent->left == a){ //check if node a was on parent's left or right node
    b->parent->left = b; //if on left, change that left node to the new node b
  } else{
    b->parent->right = b; //if on right, change that right node to the new node a
  }
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  ...
  // TODO:
  postfix_traversal(ptr->left, nth, expression); //traverse left subtree by recursively calling the postfix function
  postfix_traversal(ptr->right, nth, expression); //traverse right subtree by recursively calling the postfix function
  if (ptr->module != NULL){ //Check if module pointer exists
    expression[*nth].module = ptr->module; //set expression module to pointer module
    expression[*nth].cutline = UNDEFINED_CUTLINE; //set cutline to UNDEFINED_CUTLINE
  } else{
    expression[*nth].module = NULL; //set expression module to NULL
    expression[*nth].cutline = ptr->cutline; //set cutline to pointer cutline
  }
  *nth = *nth+1; //increment nth for the next recursive case
}
...
{
  // TODO:
  if (ptr != NULL){ //stop recursion when traverse reaches end of subtree
    if (is_leaf_node(ptr)){ //check if leaf node
      return (ptr->module->resource); //if leaf node, return the resource at the module
    } //if not leaf node, continue traversal
    return ((get_total_resource(ptr->left))+(get_total_resource(ptr->right))); //use recursion to check the left and right subtree
  }
  return 0; //return 0 if no resource value
}
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  // TODO:
  node_t *temp_n = (node_t *) malloc(sizeof(node_t)); //malloc a new node of node_t

  //Base case
  if (n == num_modules-1){
    //assign data to module pointed by temp node
    temp_n->cutline = UNDEFINED_CUTLINE; //set cutline
    temp_n->parent = par; //set parent to parent node from parameter (node above current node)
    temp_n->module = &modules[n]; //set module
    //set temp node as leaf node
    temp_n->left = NULL; 
    temp_n->right = NULL;
    return temp_n;
  }
 
  //Internal node - assign the data to the module pointer by temp node
  temp_n->cutline = V; //set cutline to vertical
  temp_n->module = NULL; //set module to NULL
  temp_n->parent = par; //set parent to parent node from parameter (node above current node)

  //Right child
  node_t *right_n = (node_t *) malloc(sizeof(node_t)); //malloc a new node of node_t as the right child of temp node
  temp_n->right = right_n; //set right pointer to the right_n node
  temp_n->right->module = &modules[n]; //assign data to the module pointer by temp_n->right
  temp_n->right->cutline = UNDEFINED_CUTLINE; //set cutline
  temp_n->right->parent = par; //set parent to the parent node from parameter (node above current node)
  
  //Left Child (recursive call){
  temp_n->left = init_slicing_tree(temp_n, n+1); //recursively create left node with current node as parent
  return temp_n;
}
